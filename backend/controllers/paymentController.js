...
export const verifyRazorpayPayment = async (req, res) => {
  try {
    ...
    // Get order details with proper population
    const order = await Order.findById(orderId)
      .populate('user', 'name email')
    ...
    // ✅ Generate order number if not exists (backup safety)
    if (!order.orderNumber) {
      order.orderNumber = await generateOrderNumber()
      console.log('✅ Order number generated during verification:', order.orderNumber)
    }
    ...
    const orderEmailData = {
      orderNumber: order.orderNumber, // ✅ FIXED: Using proper orderNumber
    ...
    try {
      await sendOrderConfirmationEmail(order.user.email, orderEmailData)
